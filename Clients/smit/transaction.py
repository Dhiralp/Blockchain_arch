...
def send_to_miner(ascii_data):
    ...
    node.sendto(ascii_data, (hostname, 5012))
...
def save_hash(json_dict):     
    ...
    #print("\nThis is hash"+str(json_object.encode('ascii')))
    hash_op = hashlib.sha256(json_object.encode()).hexdigest()
    name = str(hash_op)+'.json'
    path = get_prev_dir()
    if not os.path.isdir(path + "\\Pending transaction\\"):
        os.mkdir(path + "\\Pending transaction\\")
    fo = open(path + "\\Pending transaction\\"+name,"w")
...
def transaction(path):
    tran_dict = {}
    trans = 0
    #print("Please enter following details to start the transaction : ")
    for r,d,f in os.walk(get_prev_dir()+"//Pending Transaction"):
        trans = len(f)
        if trans >=6:
            print("Can not process more transactions")
            return
